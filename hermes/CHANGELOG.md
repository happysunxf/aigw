...
- 06:10 · `arch-benchmark` · **架构对比 / 性能基准 · 控制面 2,000 路由规模基准 + v1.0 GA 路线图**。焦点：Envoy 社区博客《Benchmarking Envoy AI Gateway Control Plane Scaling》（2026-02-23，Hrushikesh Patil）首次把"配置规模"作为独立工程指标公开，给出 2,000 × AIGatewayRoute 全量验证的基线。三组硬数字：(1) 新建 route → 可服务 ~5s，源自 `filterapi.StartConfigWatcher` 默认 5s 轮询是**设计**而非退化；(2) xDS payload 撞 4MB gRPC 默认上限，需双向调 25MB——`envoy-gateway-values.yaml` 的 `extensionManager.maxMessageSize: 25Mi` + AI Gateway controller 的 `controller.maxRecvMsgSize: "26214400"`，是"上千 AIGatewayRoute 运维"的第一道门；(3) 隐藏地雷：etcd 单对象 ~1MB 硬上限，headerMutation 等会膨胀 per-route config——意味着**2,000 路由**这个上限是 mock cassette server 下的"基线"，production 路径要重新评估。交叉对比：Envoy 走 K8s CRD 路径（v0.6 把 `AIGatewayRoute`/`AIServiceBackend`/`BackendSecurityPolicy`/`MCPRoute`/`GatewayConfig` 全部升 v1beta1，`filterConfig` 字段彻底删除，外部处理器配置迁到 `GatewayConfig` 资源）；Higress v2.2.2（2026-05-26）走 Envoy+WASM 双轨，`modelToHeader` 默认 `x-higress-llm-model-final` + `DisableReroute` 把"模型解析 → header → 限流/计量"串起来，Bedrock `/v1/messages` 改直连 Mantle 原生 API（不再 OpenAI→Converse 二次转换），nginx-rewrite 兼容插件收口 CVE-2026-42945 堆溢出；LiteLLM v1.88-rc.1 走 Python 控制面 + `feat(otel): typed semconv-aligned OpenTelemetry instrumentation`（PR #28909，2026-05-30 合入），`LITELLM_OTEL_V2` 开关默认关、`set_request_root_span` 显式锚定 SERVER root span 解决 ambient parent 错乱、v1.88-rc.2/rc.3 三个 rc 都集中在 `GHSA-q775` session-token 豁免硬化。v1.0 GA 路线（issue #2083 锁定）：CRD 升 v1beta1、目标 EG 1.8、6/12 v1.0-rc1 分支冻结、6/30 GA；**extproc 暂不上 dynamic modules**是已知不退清单。详见 [report](reports/2026-06-06-0610-aigw-arch-benchmark-r5.md)。
